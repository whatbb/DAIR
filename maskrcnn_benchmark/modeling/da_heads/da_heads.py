...
class MULTDAImgHead(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        """
        super(MULTDAImgHead, self).__init__()
        

        self.conv1_da = nn.Conv2d(in_channels[3], in_channels[3], kernel_size=1, stride=1)
        self.conv2_da = nn.Conv2d(in_channels[3], 1, kernel_size=1, stride=1)

        self.conv3_da = nn.Conv2d(in_channels[2], in_channels[2],  kernel_size=1, stride=1)
        self.conv4_da = nn.Conv2d(in_channels[2], 1,  kernel_size=1, stride=1)

        self.conv5_da = nn.Conv2d(in_channels[1], in_channels[1], kernel_size=1, stride=1)
        self.conv6_da = nn.Conv2d(in_channels[1], 1, kernel_size=1, stride=1)

        self.conv7_da = nn.Conv2d(in_channels[0], in_channels[1], kernel_size=1, stride=1)
        self.conv8_da = nn.Conv2d(in_channels[1], 1, kernel_size=1, stride=1)

        for l in [self.conv1_da,self.conv2_da, self.conv3_da,self.conv4_da, self.conv5_da,self.conv6_da, self.conv7_da,self.conv8_da]:
    ...
    def forward(self, features):
        ...
        t = F.relu(self.conv1_da(features[0]))
        t = F.relu(self.conv2_da(t))

        img_features.append(t)

        t = F.relu(self.conv3_da(features[1]))
        t = F.relu(self.conv4_da(t))

        img_features.append(t)

        t = F.relu(self.conv5_da(features[2]))
        t = F.relu(self.conv6_da(t))

        img_features.append(t)

        t = F.relu(self.conv7_da(features[3]))
        t = F.relu(self.conv8_da(t))
...
class DomainAdaptationModule(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        in_channels = [out_channels//2**i for i in range(self.re_features_nums)]

        self.imghead = MULTDAImgHead(in_channels)
        self.inshead = DAInsHead(num_ins_inputs)
        self.loss_evaluator = make_da_heads_loss_evaluator(cfg)

    def forward(self, img_features, da_ins_feature, da_ins_labels, targets=None , iteration=None):
        """
        Arguments:
        ...
        if self.training:
            da_img_loss, da_ins_loss,da_consist_loss= self.loss_evaluator(
                da_img_features, da_ins_features, da_ins_labels, targets , da_img_consist_features,da_ins_consist_features
            )
            losses = {}
