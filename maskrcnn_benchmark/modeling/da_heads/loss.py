...
class DALossComputation(object):
    ...
    def __init__(self, cfg):
        ...
        self.level_loss_weight = cfg.MODEL.INVERSION.DA_IMS_LEVEL_LOSS_WEIGHT
    ...
    def __call__(self, da_img, da_ins, da_ins_labels, targets,da_img_consist_features,da_ins_consist_features):
        """
        Arguments:
            da_img (list[Tensor])
        ...
        da_img_loss1 = F.binary_cross_entropy_with_logits(
                da_img_flattened[0], da_img_labels_flattened[0]
            )
        da_img_loss2 = F.binary_cross_entropy_with_logits(
                da_img_flattened[1], da_img_labels_flattened[1]
            )
        da_img_loss3 = F.binary_cross_entropy_with_logits(
                da_img_flattened[2], da_img_labels_flattened[2]
            )
        da_img_loss4 = F.binary_cross_entropy_with_logits(
                da_img_flattened[3], da_img_labels_flattened[3]
            )


        da_img_loss = da_img_loss1 * self.level_loss_weight[0] +da_img_loss2 * self.level_loss_weight[1] + da_img_loss3 * self.level_loss_weight[2] + da_img_loss4 * self.level_loss_weight[3]
        
        da_ins_loss = F.binary_cross_entropy_with_logits(
            torch.squeeze(da_ins), da_ins_labels.type(torch.cuda.FloatTensor)
        )
        da_consist_loss = consistency_loss(da_img_consist_features, da_ins_consist_features, da_ins_labels, size_average=True)
